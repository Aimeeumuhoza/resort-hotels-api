...
class userController {
    ...
    static async forgotPassword(req, res) {
        try {
            ...
            if (!user) {
                res.status(201).json({ error: "there is no user with this email" })
            }
            const userInfo = {
                token: jwt.sign({ _id: user._id }, process.env.RESET_PASSWORD_KEY),
                email: user.email
            }
            //   console.log(userInfo);
            await mailer({email:userInfo.email, token:userInfo.token}, 'forgotPassword').then(() => {
    ...
    static async resetPassword(req, res) {
        try {
            ...
            if (token) {
                ...
                const userInfos = await User.findOne({ _id: data._id.toString() })
                ...
                const user = await User.findOne({_id: userInfos._id})
                console.log(user);
